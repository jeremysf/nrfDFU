...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        if( argc < 2 ) {
            fprintf(stderr, "usage:\n\t%s <command>\ncommands:\n\tupdate <application.hex> [uuid]\n\tdiscover\n", argv[0]);
            return 1;
        }
        NDDFUController* dfuController = [[NDDFUController alloc] init];
        if( strcmp(argv[1], "update") == 0 ) {
            if( argc < 3 ) {
                fprintf(stderr, "error: missing application file name command line argument.\n");
                return 1;
            }
            NSString* applicationFileName = [NSString stringWithUTF8String:argv[2]];
            NSString* deviceUUID = nil;
            if( argc > 3 ) {
                deviceUUID = [NSString stringWithUTF8String:argv[3]];
            }
